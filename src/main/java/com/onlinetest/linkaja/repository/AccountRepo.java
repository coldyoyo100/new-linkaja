...
public interface AccountRepo extends JpaRepository<AccountModel, Long> {
	...
	AccountModel getInfoByAccNo(@Param("accNo") String accNo);
	
	@Query(value = "SELECT 1 FROM account acc WHERE acc.account_number = :accNo", nativeQuery = true)
	String checkAccount(@Param("accNo")String accNo);
	
	@Query(value ="SELECT count(acc.1) FROM account acc", nativeQuery = true)
	int countSize();
}
